...
echo  "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games AS w ON teams.team_id = w.winner_id FULL JOIN games AS o ON teams.team_id = o.opponent_id WHERE (w.year = 2014 AND w.round='Eighth-Final') OR (o.year = 2014 AND o.round='Eighth-Final') ORDER BY name")"
